...
// NewDefaultMiddleware creates a middleware instance and uses resFun when provided 创建默认中间件实例，传入 resFun 时使用自定义响应函数
...
// GetRequestToken extracts token from Authorization header or token param 从 Authorization 头或 token 参数中提取 Token
