// Author daixk 2025-10-28 22:00:20 作者 daixk 2025-10-28 22:00:20
package dtoken_gf
...
type RenewPoolConfig struct {
	...
	PrintStatusInterval time.Duration // Interval for periodic status printing, 0 means disabled 定时打印池状态的间隔，0 表示关闭
...
func (m *RenewPoolManager) autoScale() {
	...
	for {
		...
		case <-ticker.C:
			...
			switch {
			// Expand if usage exceeds threshold and capacity is below MaxSize 使用率超过扩容阈值且容量小于最大值时扩容
